...
export default function ModelUpdateForm(props) {
  ...
  const resetStateValues = () => {
    ...
    setDashboardLayout(cleanValues.dashboardLayouts ?? []);
    setCurrentDashboardLayoutValue("");
    setErrors({});
  };
  const [modelRecord, setModelRecord] = React.useState(modelModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(Model, idProp)
        : modelModelProp;
      setModelRecord(record);
    };
    queryData();
  }, [idProp, modelModelProp]);
  React.useEffect(resetStateValues, [modelRecord]);
  const [currentDashboardLayoutValue, setCurrentDashboardLayoutValue] =
    React.useState("");
  const dashboardLayoutRef = React.createRef();
  const validations = {
    name: [],
    description: [],
    ...
    >
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || modelModelProp)}
